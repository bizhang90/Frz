(() => {
  ...
  let data=null, galleryFilter='all', galleryLimit=8, menuLimit=12, menuType='regular', pendingMenuType=null, dialogItems=[], dialogIndex=0;
  ...
  function menuItems(type=menuType){if(type==='group'&&data?.groupMenu?.length)return data.groupMenu;return data?.menu||[];}
  function updateMenuTabs(){qsa('[data-menu-tab]').forEach(btn=>{const active=btn.dataset.menuTab===menuType;btn.classList.toggle('active',active);btn.setAttribute('aria-selected',String(active));});}
  function setMenuType(type){menuType=(type==='group'&&data?.groupMenu?.length)?'group':'regular';menuLimit=12;updateMenuTabs();}
  function renderMenu(){const grid=qs('[data-menu-grid]'), more=qs('[data-menu-more]'), status=qs('[data-menu-status]'), title=qs('[data-menu-title]');if(!grid)return;const items=menuItems();grid.replaceChildren();items.slice(0,menuLimit).forEach((item,index)=>{const b=document.createElement('button');b.type='button';b.className='menu-page';b.innerHTML=`<img src="${item.thumb}" alt="${item.alt}" width="360" height="480" loading="lazy" decoding="async"><span>Trang ${item.page}</span>`;b.addEventListener('click',()=>openImageCollection(items,index));grid.append(b);});if(title)title.textContent=menuType==='group'?'Menu cơm đoàn THÚNG View Hồ Tôm':'Thực đơn gọi món THÚNG View Hồ Tôm';if(status)status.textContent=`Đang hiển thị ${Math.min(menuLimit,items.length)}/${items.length} trang`;if(more)more.hidden=items.length<=menuLimit;}
  const menuBrowser=qs('[data-menu-browser]'), menuCopy=qs('.menu-copy'), menuPreview=qs('.menu-preview');
  const openMenu=(type='regular')=>{if(!menuBrowser)return;if(!data){pendingMenuType=type;return;}setMenuType(type);menuBrowser.hidden=false;if(menuCopy)menuCopy.hidden=true;if(menuPreview)menuPreview.hidden=true;renderMenu();menuBrowser.scrollIntoView({behavior:'smooth',block:'start'});};
  const closeMenu=()=>{if(!menuBrowser)return;menuBrowser.hidden=true;if(menuCopy)menuCopy.hidden=false;if(menuPreview)menuPreview.hidden=false;qs('#thuc-don')?.scrollIntoView({behavior:'smooth',block:'start'});};
  qsa('[data-menu-open]').forEach(btn=>btn.addEventListener('click',()=>openMenu(btn.dataset.menuType||'regular')));
  qsa('[data-menu-tab]').forEach(btn=>btn.addEventListener('click',()=>{setMenuType(btn.dataset.menuTab);renderMenu();}));
  qs('[data-menu-close]')?.addEventListener('click',closeMenu);qs('[data-menu-more]')?.addEventListener('click',()=>{menuLimit+=12;renderMenu();});
  qsa('[data-group-menu-open]').forEach(btn=>btn.addEventListener('click',()=>openMenu('group')));
  ...
  fetch(jsonUrl,{credentials:'same-origin',cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`HTTP ${r.status}`);return r.json();}).then(json=>{data=json;renderGallery();renderVideos();bindVipPreviews();const regularCount=data.menu?.length||0, groupCount=data.groupMenu?.length||0;qsa('[data-menu-count]').forEach(el=>el.textContent=regularCount+groupCount);qsa('[data-menu-regular-count]').forEach(el=>el.textContent=regularCount);qsa('[data-menu-group-count]').forEach(el=>el.textContent=groupCount);qsa('[data-menu-tab="group"],[data-menu-open][data-menu-type="group"],[data-group-menu-open]').forEach(el=>{if(!groupCount)el.hidden=true;});if(pendingMenuType){const requested=pendingMenuType;pendingMenuType=null;openMenu(requested);}}).catch(err=>{console.error('Không tải được dữ liệu nhà hàng',err);const grid=qs('[data-gallery-grid]');if(grid)grid.innerHTML='<p>Hình ảnh đang được cập nhật. Vui lòng liên hệ nhà hàng để được hỗ trợ.</p>';});
})();
