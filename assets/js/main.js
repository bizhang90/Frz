const projects = [
  {
    id: 'venus',
    category: 'stay',
    categoryLabel: 'Resort gần biển',
    name: 'Venus Mũi Né Resort',
    description: 'Hồ bơi hướng biển, phòng view biển và view vườn, bữa sáng, BBQ cùng nhiều trải nghiệm dành cho gia đình, nhóm bạn và khách đoàn.',
  ...
  },
  {
    ...
    image: '/assets/img/brands/volga-hotel-apartment.webp',
    location: 'Mũi Né',
    href: ''
  },
  {
    ...
    description: 'Điểm lưu trú tiện lợi dành cho khách du lịch, người đi công tác, cặp đôi và nhóm bạn tại Phan Thiết – Mũi Né.',
  ...
  },
  {
    id: 'saigonpho',
    category: 'social',
    categoryLabel: 'Beer Garden & Karaoke',
    name: 'Sài Gòn Phố',
  ...
  },
  {
    id: 'chammm',
    category: 'food',
    categoryLabel: 'Ẩm thực địa phương',
...
function observeRevealElements(root = document) {
  const elements = [...root.querySelectorAll('[data-reveal]:not(.is-observed)')];
...
function renderProjects(filter = 'all') {
  ...
  grid.innerHTML = data.map((project, index) => {
    const action = project.href
      ? `<a class="project-link" href="${project.href}">Khám phá <span>→</span></a>`
      : '';
    return `<article class="project-card" data-category="${project.category}" data-reveal data-delay="${Math.min(index * 55, 275)}">
      ...
      <div class="project-body">
        ...
        <div class="project-footer"><span class="project-location">📍 ${project.location}</span>${action}</div>
      </div>
    </article>`;
  }).join('');
...
function activateFilter(filter, shouldScroll = false) {
  ...
  if (shouldScroll) document.querySelector('#diem-den')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
}
...
document.querySelectorAll('[data-nav-filter]').forEach(link => link.addEventListener('click', () => {
  activateFilter(link.dataset.navFilter);
}));
